...
class ElementDefinitionType {
    ...
    get code() {
        var _a, _b, _c;
        const fhirTypeExtension = (_a = this.extension) === null || _a === void 0 ? void 0 : _a.find(ext => ext.url === 'http://hl7.org/fhir/StructureDefinition/structuredefinition-fhir-type');
        return _c = (_b = fhirTypeExtension) === null || _b === void 0 ? void 0 : _b.valueUrl, (_c !== null && _c !== void 0 ? _c : this._code);
    }
    set code(c) {
        this._code = c;
    }
    ...
    toJSON() {
        ...
        const elDefTypeJSON = Object.assign({ code: this.getActualCode() }, elDefTypeClone);
...
class ElementDefinition {
    ...
    getOwnStructureDefinition(fisher) {
        if (this._edStructureDefinition == null) {
            this._edStructureDefinition = StructureDefinition_1.StructureDefinition.fromJSON(fisher.fishForFHIR('ElementDefinition', utils_1.Type.Type));
        }
    ...
    hasDiff() {
        ...
        return PROPS.some(prop => {
            ...
            // @ts-ignore
            return prop && !lodash_1.isEqual(this[prop], original[prop]);
        });
    }
    /**
    ...
    calculateDiff() {
        ...
        for (let prop of PROPS) {
            ...
            // @ts-ignore
            if (prop && !lodash_1.isEqual(this[prop], original[prop])) {
                // @ts-ignore
                diff[prop] = lodash_1.cloneDeep(this[prop]);
            }
        }
     ...
     */
    applyConstraint(invariant, source) {
        const constraint = Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign({}, (invariant.name && { key: invariant.name })), (invariant.severity && { severity: invariant.severity.code })), (invariant.description && { human: invariant.description })), (invariant.expression && { expression: invariant.expression })), (invariant.xpath && { xpath: invariant.xpath })), (source && { source }));
        if (this.constraint) {
    ...
    setInstancePropertyByPath(path, value, fisher) {
        common_1.setPropertyOnDefinitionInstance(this, path, value, fisher);
    }
    getSlices() {
        return this.structDef.elements.filter(e => e.id !== this.id && e.path === this.path && e.id.startsWith(this.id));
    }
    /**
    ...
    constrainCardinality(min, max) {
        ...
        if (!isUnbounded && min > maxInt) {
            throw new errors_1.InvalidCardinalityError(min, max);
        }
        // Check to ensure min >= existing min
        if (this.min != null && min < this.min) {
            throw new errors_1.WideningCardinalityError(this.min, this.max, min, max);
        }
        ...
        if (this.slicing) {
            ...
            if (!isUnbounded && overMaxChild) {
                throw new errors_1.InvalidMaxOfSliceError(overMaxChild.max, overMaxChild.sliceName, max);
            }
        }
        ...
        if (connectedElements.length > 0) {
            ...
            connectedElements
                ...
                if (ce.min != null && ce.min < min) {
                    throw new errors_1.NarrowingRootCardinalityError(this.path, ce.id, min, max, ce.min, (_a = ce.max, (_a !== null && _a !== void 0 ? _a : '*')));
                }
            });
            connectedElements.forEach(ce => {
                ...
                if (ce.max != null && ce.max != '*' && maxInt != null && maxInt < parseInt(ce.max)) {
                    throw new errors_1.NarrowingRootCardinalityError(this.path, ce.id, min, max, (_a = ce.min, (_a !== null && _a !== void 0 ? _a : 0)), ce.max);
                }
            });
        }
     ...
     */
    findConnectedElements(postPath = '') {
        const connectedElements = this.getSlices()
            ...
            .filter(e => e);
        if (this.parent()) {
            const [parentPath] = common_1.splitOnPathPeriods(this.path).slice(-1);
            return connectedElements.concat(this.parent().findConnectedElements(`.${parentPath}${postPath}`));
        }
        else {
     ...
     */
    checkSumOfSliceMins(newSlicedElementMax, sliceMinIncrease = 0) {
        const slices = this.getSlices();
        ...
        if (newSlicedElementMax !== '*' && sumOfMins > parseInt(newSlicedElementMax)) {
            throw new errors_1.InvalidSumOfSliceMinsError(sumOfMins, newSlicedElementMax, this.id);
        }
        else {
            return sumOfMins;
        }
    ...
    constrainType(rule, fisher, target) {
        ...
        for (const type of this.type) {
            ...
            if (!typeMatches.has(type.code)) {
                newTypes.push(lodash_1.cloneDeep(type));
                continue;
            }
        ...
        if (obsoleteChoices.length > 0) {
            utils_1.logger.error(`Type constraint on ${this.path} makes rules in ${this.structDef.name} obsolete for choices: ${obsoleteChoices.join(', ')}`, rule.sourceInfo);
        }
        ...
        if (connectedElements.length > 0) {
            ...
            const connectedTypeChanges = new Map();
            connectedElements.forEach(ce => {
                ...
                if (intersection.length > 0) {
                    ...
                    if (obsoleteConnections.length > 0) {
                        utils_1.logger.error(`Type constraint on ${rule.path} makes rules in ${ce.structDef.name} obsolete for choices: ${obsoleteConnections.join(', ')}`, rule.sourceInfo);
                    }
                    else {
                        throw new errors_1.SliceTypeRemovalError(rule.path, ce.id);
                    }
                }
            });
     ...
     */
    getTargetType(target, fisher) {
        var _a, _b;
        let targetType;
        if (target) {
            const targetSD = fisher.fishForMetadata(target, utils_1.Type.Resource, utils_1.Type.Type, utils_1.Type.Profile, utils_1.Type.Extension);
            if (targetSD == null) {
                throw new errors_1.TypeNotFoundError(target);
            }
            ...
            targetType = lodash_1.cloneDeep(this.type.find(t => {
                var _a, _b;
                return t.code === targetSD.id || ((_a = t.profile) === null || _a === void 0 ? void 0 : _a.includes(targetSD.url)) || ((_b = t.targetProfile) === null || _b === void 0 ? void 0 : _b.includes(targetSD.url));
            }));
            if (!targetType) {
                throw new errors_1.InvalidTypeError(target, this.type);
            }
            // Re-assign the targetProfiles/profiles as appopriate to remove non-targets
            if ((_a = targetType.profile) === null || _a === void 0 ? void 0 : _a.includes(targetSD.url)) {
                targetType.profile = [targetSD.url];
            }
            else if ((_b = targetType.targetProfile) === null || _b === void 0 ? void 0 : _b.includes(targetSD.url)) {
     ...
     */
    findTypeMatch(type, targetTypes, fisher) {
        let matchedType;
        ...
        if (lodash_1.isEmpty(lineage)) {
            throw new errors_1.TypeNotFoundError(type.type);
        }
        ...
        for (const md of lineage) {
            if (type.isReference) {
                ...
                matchedType = targetTypes.find(t2 => t2.code === 'Reference' &&
                ...
                matchedType = targetTypes.find(t2 => {
                    var _a;
                    const matchesUnprofiledResource = t2.code === md.id && lodash_1.isEmpty(t2.profile);
                    const matchesProfile = t2.code === md.sdType && ((_a = t2.profile) === null || _a === void 0 ? void 0 : _a.includes(md.url));
        ...
        if (!matchedType) {
            throw new errors_1.InvalidTypeError(type.isReference ? `Reference(${type.type})` : type.type, targetTypes);
        }
        return {
            metadata: lineage[0],
    ...
    getTypeLineage(type, fisher) {
        ...
        while (currentType != null) {
            ...
            currentType = (_a = result) === null || _a === void 0 ? void 0 : _a.parent;
        }
        return results;
    }
    /**
     ...
     *   to element types
     */
    applyProfiles(newType, targetType, matches) {
        var _a, _b, _c, _d;
        const matchedProfiles = [];
        const matchedTargetProfiles = [];
        ...
        if (targetType) {
            if (!lodash_1.isEmpty(matchedTargetProfiles)) {
                const targetIdx = (_a = newType.targetProfile) === null || _a === void 0 ? void 0 : _a.indexOf(targetType.targetProfile[0]);
                if (targetIdx != null && targetIdx > -1) {
                    newType.targetProfile.splice(targetIdx, 1, ...matchedTargetProfiles);
                }
                else {
                    newType.targetProfile = (_b = newType.profile, (_b !== null && _b !== void 0 ? _b : []));
                    newType.targetProfile.push(...matchedTargetProfiles);
                }
            }
            if (!lodash_1.isEmpty(matchedProfiles)) {
                const targetIdx = (_c = newType.profile) === null || _c === void 0 ? void 0 : _c.indexOf(targetType.profile[0]);
                if (targetIdx != null && targetIdx > -1) {
                    newType.profile.splice(targetIdx, 1, ...matchedProfiles);
                }
                else {
                    newType.profile = (_d = newType.profile, (_d !== null && _d !== void 0 ? _d : []));
    ...
    findTypeIntersection(leftTypes, rightTypes, targetType, fisher) {
        const intersection = [];
        let match;
    ...
    applyTypeIntersection(type, targetType, matches) {
        const intersection = [];
        const currentTypeMatches = new Map();
        for (const match of matches) {
        ...
        for (const [typeCode, currentMatches] of currentTypeMatches) {
            const newType = lodash_1.cloneDeep(type);
     ...
     */
    applyFlags(mustSupport, summary, modifier, trialUse, normative, draft) {
        let newStatusExtension = null;
        if (trialUse) {
            newStatusExtension = {
        ...
        if (normative) {
            if (newStatusExtension) {
                throw new errors_1.MultipleStandardsStatusError(this.id);
            }
            newStatusExtension = {
                url: 'http://hl7.org/fhir/StructureDefinition/structuredefinition-standards-status',
                valueCode: 'normative'
        ...
        if (newStatusExtension) {
            if (this.extension) {
                const oldStatus = this.extension.findIndex(extension => extension.url ==
     ...
     */
    bindToVS(vsURI, strength, units = false) {
        var _a, _b, _c, _d, _e, _f;
        // Check if this is a valid type to be bound against
        const validTypes = this.findTypesByCode('code', 'Coding', 'CodeableConcept', 'Quantity', 'string', 'uri');
        if (lodash_1.isEmpty(validTypes)) {
            throw new errors_1.CodedTypeNotFoundError(this.type ? this.type.map(t => t.code) : []);
        }
        ...
        if (this.binding && this.binding.strength) {
            if (strengths.indexOf(strength) < strengths.indexOf(this.binding.strength)) {
                throw new errors_1.BindingStrengthError(this.binding.strength, strength);
            }
        }
        const connectedElements = this.findConnectedElements();
        connectedElements.forEach(ce => {
            var _a;
            if (vsURI == ((_a = ce.binding) === null || _a === void 0 ? void 0 : _a.valueSet)) {
                try {
        ...
        if (((_b = (_a = listElement) === null || _a === void 0 ? void 0 : _a.binding) === null || _b === void 0 ? void 0 : _b.valueSet) == vsURI &&
            strengths.indexOf(strength) < strengths.indexOf((_d = (_c = listElement) === null || _c === void 0 ? void 0 : _c.binding) === null || _d === void 0 ? void 0 : _d.strength)) {
            throw new errors_1.BindingStrengthError((_f = (_e = listElement) === null || _e === void 0 ? void 0 : _e.binding) === null || _f === void 0 ? void 0 : _f.strength, strength);
        }
        // Canonical URLs may include | to specify version: https://www.hl7.org/fhir/references.html#canonical
        if (!valid_url_1.isUri(vsURI.split('|')[0])) {
            throw new errors_1.InvalidUriError(vsURI);
        }
        // We're good.  Bind it.
        this.binding = {
            strength,
        ...
        if (units && !validTypes.find(t => t.code === 'Quantity')) {
            throw new errors_1.InvalidUnitsError(this.id);
        }
    }
    /**
     ...
     * @throws {InvalidUnitsError} when the "units" keyword is used on a non-Quantity type
     */
    fixValue(value, exactly = false, units = false) {
        var _a;
        let type;
        if (value instanceof fshtypes_1.FshCode) {
            type = 'Code';
        }
        else if (value instanceof fshtypes_1.FshQuantity) {
            type = 'Quantity';
        }
        else if (value instanceof fshtypes_1.FshRatio) {
            type = 'Ratio';
        }
        else if (value instanceof fshtypes_1.FshReference) {
            type = 'Reference';
        }
        else if (typeof value === 'object') {
            type = (_a = value.constructor) === null || _a === void 0 ? void 0 : _a.name;
        }
        else {
            type = typeof value;
        }
        ...
        if (!this.hasSingleType()) {
            throw new errors_1.NoSingleTypeError(type);
        }
        ...
        if (!exactly) {
            ...
            if (fixedField) {
                throw new errors_1.FixedToPatternError(fixedField[0]);
            }
        }
        ...
        switch (type) {
            case 'boolean':
                ...
                break;
            case 'number':
                this.fixNumber(value, exactly);
                break;
            case 'string':
                this.fixString(value, exactly);
                break;
            case 'Code':
                this.fixFshCode(value, exactly);
                break;
            case 'Quantity':
                value = value;
            ...
            case 'Reference':
                ...
                break;
            default:
                throw new errors_1.MismatchedTypeError(type, value, this.type[0].code);
        }
     ...
     * @throws {MismatchedTypeError} when the value does not match the type of the ElementDefinition
     */
    fixFHIRValue(fshValue, fhirValue, exactly, type) {
        var _a, _b;
        if (this.type[0].code !== type) {
            throw new errors_1.MismatchedTypeError(type, fshValue, this.type[0].code);
        }
        // Find the fixed[x] and pattern[x] variables to use
        const fixedX = `fixed${lodash_1.upperFirst(type)}`;
        const patternX = `pattern${lodash_1.upperFirst(type)}`;
        // Find any currently fixed values
        const currentElementValue = (_b = (_a = this[fixedX], (_a !== null && _a !== void 0 ? _a : this[patternX])), (_b !== null && _b !== void 0 ? _b : this.fixedByAnyParent()));
        // For complex types, use isMatch to check if they are a subset, otherwise use isEqual
        const compareFn = typeof fhirValue === 'object' ? lodash_1.isMatch : lodash_1.isEqual;
        if (currentElementValue != null && !compareFn(fhirValue, currentElementValue)) {
            ...
            throw new errors_1.ValueAlreadyFixedError(fshValue, type, JSON.stringify(currentElementValue));
        }
    ...
    fixedByDirectParent() {
        ...
        const fixedKey = parent
            ...
            : null;
        if (fixedKey) {
            const fixedValue = parent[fixedKey];
        ...
        if (parent == null) {
            ...
            if (fixedValue == null) {
                ...
                if (Array.isArray(parentValue)) {
                    ...
                    fixedValue = (_a = parentValue) === null || _a === void 0 ? void 0 : _a[childIndex];
                }
            }
            return fixedValue;
        }
    ...
    fixNumber(value, exactly = false) {
        ...
        if (type === 'decimal' ||
            ...
            throw new errors_1.MismatchedTypeError('number', value, type);
        }
    }
    /**
    ...
    fixString(value, exactly = false) {
        ...
        if (type === 'string' ||
            ...
            (type === 'instant' &&
                /^([0-9]([0-9]([0-9][1-9]|[1-9]0)|[1-9]00)|[1-9]000)-(0[1-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1])T([01][0-9]|2[0-3]):[0-5][0-9]:([0-5][0-9]|60)(\.[0-9]+)?(Z|(\+|-)((0[0-9]|1[0-3]):[0-5][0-9]|14:00))$/.test(value)) ||
            (type === 'date' &&
                /^([0-9]([0-9]([0-9][1-9]|[1-9]0)|[1-9]00)|[1-9]000)(-(0[1-9]|1[0-2])(-(0[1-9]|[1-2][0-9]|3[0-1]))?)?$/.test(value)) ||
            (type === 'dateTime' &&
                /^([0-9]([0-9]([0-9][1-9]|[1-9]0)|[1-9]00)|[1-9]000)(-(0[1-9]|1[0-2])(-(0[1-9]|[1-2][0-9]|3[0-1])(T([01][0-9]|2[0-3]):[0-5][0-9]:([0-5][0-9]|60)(\.[0-9]+)?(Z|(\+|-)((0[0-9]|1[0-3]):[0-5][0-9]|14:00)))?)?)?$/.test(value)) ||
            (type === 'time' &&
            ...
            this[exactly ? 'fixedXhtml' : 'patternXhtml'] = html_minifier_1.minify(value, { collapseWhitespace: true });
        }
        else {
            throw new errors_1.MismatchedTypeError('string', value, type);
        }
    }
    checkXhtml(value) {
        try {
     ...
     */
    checkFixResource(value) {
        if (!this.hasSingleType()) {
            throw new errors_1.NoSingleTypeError('Resource');
        }
        const type = this.type[0].code;
        if (type === 'Resource') {
            return value;
            ...
            throw new errors_1.MismatchedTypeError('Resource', value.id, type);
        }
    }
    /**
     ...
     */
    fixFshCode(code, exactly = false) {
        var _a, _b, _c, _d;
        if (code.system && !valid_url_1.isUri(code.system.split('|')[0])) {
            throw new errors_1.InvalidUriError(code.system);
        }
        const type = this.type[0].code;
        if (type === 'code' || type === 'string' || type === 'uri') {
            ...
            const existing = (_b = (_a = this.fixedQuantity, (_a !== null && _a !== void 0 ? _a : this.patternQuantity)), (_b !== null && _b !== void 0 ? _b : this.fixedByAnyParent()));
            const quantity = code.toFHIRQuantity();
            if (((_c = existing) === null || _c === void 0 ? void 0 : _c.value) != null) {
                quantity.value = existing.value;
            }
            if (((_d = existing) === null || _d === void 0 ? void 0 : _d.comparator) != null) {
                quantity.comparator = existing.comparator;
            }
            this.fixFHIRValue(code.toString(), quantity, exactly, 'Quantity');
        }
        else {
            throw new errors_1.CodedTypeNotFoundError([type]);
        }
    }
    /**
    ...
    hasSingleType() {
        var _a;
        const types = (_a = this.type, (_a !== null && _a !== void 0 ? _a : []));
     ...
     */
    applyMapping(identity, map, comment, language) {
        if (identity == null || map == null) {
            throw new errors_1.InvalidMappingError();
        }
        if (!primitiveTypes_1.idRegex.test(identity)) {
            throw new errors_1.InvalidFHIRIdError(identity);
        }
        this.mapping.push(Object.assign(Object.assign({ identity,
            map }, (comment && { comment })), (language && { language: language.code })));
    }
    /**
    ...
    unfold(fisher) {
        ...
        if ((((_a = this.type) === null || _a === void 0 ? void 0 : _a.length) === 1 && (_b = this.type[0].profile, (_b !== null && _b !== void 0 ? _b : [])).length <= 1) ||
            ...
            if (this.contentReference) {
                ...
                const json = fisher.fishForFHIR(type, utils_1.Type.Resource);
                if (json) {
                    const def = StructureDefinition_1.StructureDefinition.fromJSON(json);
                    ...
                    newElements = (_c = referencedElement) === null || _c === void 0 ? void 0 : _c.children().map(e => {
            ...
            if (newElements.length === 0) {
                ...
                const type = (_e = (_d = this.type[0].profile) === null || _d === void 0 ? void 0 : _d[0], (_e !== null && _e !== void 0 ? _e : this.type[0].code));
                const json = fisher.fishForFHIR(type, utils_1.Type.Resource, utils_1.Type.Type, utils_1.Type.Profile, utils_1.Type.Extension);
                if (json) {
                    const def = StructureDefinition_1.StructureDefinition.fromJSON(json);
                    if (def.inProgress) {
                        utils_1.logger.debug(`Warning: Circular relationship detected between ${(_f = this.structDef) === null || _f === void 0 ? void 0 : _f.name} and ${def.name}. As a result, it is possible that the definition of ${(_g = this.structDef) === null || _g === void 0 ? void 0 : _g.name} may be based on incomplete components of ${def.name}.`);
                    }
     ...
     */
    sliceIt(discriminatorType, discriminatorPath, ordered, rules) {
        if (!this.slicing || !this.slicing.discriminator) {
            ...
            if (this.slicing.ordered && ordered === false) {
                throw new errors_1.SlicingDefinitionError('ordered', true, false);
            }
            // Rules cannot be changed from closed to open/openAtEnd or openAtEnd to open
            if ((this.slicing.rules === 'closed' && rules !== 'closed') ||
                (this.slicing.rules === 'openAtEnd' && rules === 'open')) {
                throw new errors_1.SlicingDefinitionError('rules', this.slicing.rules, rules);
            }
    ...
    addSlice(name, type) {
        if (!this.slicing) {
            throw new errors_1.SlicingNotDefinedError(this.id, name);
        }
    ...
    clone(clearOriginal = true) {
        ...
        this.structDef = null;
        const clone = lodash_1.cloneDeep(this);
        ...
        for (let prop of PROPS) {
            ...
            if (prop && this[prop] !== undefined) {
                if (prop === 'type') {
                    ...
                    // @ts-ignore
                    j[prop] = lodash_1.cloneDeep(this[prop]);
                }
            }
        }
        return j;
     ...
     */
    static fromJSON(json, captureOriginal = true) {
        ...
        for (let prop of PROPS) {
            ...
            if (prop && json[prop] !== undefined) {
                if (prop === 'type') {
                    ...
                    // @ts-ignore
                    ed[prop] = lodash_1.cloneDeep(json[prop]);
                }
            }
        }
        if (captureOriginal) {
